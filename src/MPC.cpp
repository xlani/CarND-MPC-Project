...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Setup Constraints
    // Initial constraints
    // Because cost is stored in fg[0], add + 1 to every index
    ...
    for (int t = 1; t < N; t++) {
        ...
        AD<double> fdx = coeffs[1] + coeffs[2] * 2 * x0 + \
                         coeffs[3] * 3 * CppAD::pow(x0, 2);
        AD<double> psides0 = CppAD::atan(fdx);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //use state variables for better readability
  double x = state[0];
  double y = state[1];
  double psi = state[2];
  double v = state[3];
  ...
  //Bounds for variables to search in
